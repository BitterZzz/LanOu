<template>
  <div id="user">
    ...
    <sorter :pageMsg="sortPage"
            @Information="showUserInfo"></sorter>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    serachAction() {
      ...
      console.log(searchText.value.length, '1232323')
      if (valuePhone.test(searchText.value)) {
        console.log(1)
        this.cellPhoneSear()
      } else if (searchText.value === '') {
        this.allSear()
      } else if (searchText.value !== '' && searchText.value !== valuePhone) {
        this.reacNameSear()
        console.log(90)
      }
    },
    ...
    reacNameSear() {
      ...
      this.$get(this.$api.getLanOuUserInfo, {
        ...
        console.log(res.data.data.list,"233")
        this.tableData = res.data.data.list;
      })
    },
    // 封装根据手机号cellPhone查询
